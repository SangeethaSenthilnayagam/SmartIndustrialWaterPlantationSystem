...
// ═══════════════════════════════════════════════════════════════════════════════
// MAIN COMPONENT – NOW PURELY PRESENTATIONAL
// ═══════════════════════════════════════════════════════════════════════════════
function JsonDisplay({
    ...
    Object.entries(flows).forEach(([fmId, flowVal]) => {
      ...
      if (fmId === 'flowmeter-001') {
        (PIPE_GROUPS[fmId] || []).forEach(pipeId => {
          ...
          applyPipeFlow(graph.getCell(pipeId), shouldFlow, flowVal);
        });

        const allFull = Object.keys(INITIAL_LEVELS).every(id => (levels[id] ?? 0) >= 100);
        applyPipeFlow(graph.getCell('header-pipe-main'), flowing && masterOpen && !allFull, flowVal);
      } else {
        (PIPE_GROUPS[fmId] || []).forEach(pipeId => {
          const linkedTank = BRANCH_PIPE_TO_TANK[pipeId];
          const tankFull = linkedTank ? (levels[linkedTank] ?? 0) >= 100 : false;
          applyPipeFlow(graph.getCell(pipeId), flowing && !tankFull, flowVal);
        });
      }
    });
  }, [flows, levels, valveStates]);

  // ── Initial JointJS setup (unchanged) ───────────────────────────────────────
  useEffect(() => {
  ...
  if (tapZoneInfo) {
    ...
    return (
      <div style={{ position: 'relative', width: '100%', height: '100%', background: '#0d1b2e' }}>
        <button onClick={() => setTapZoneInfo(null)} style={{ position: 'absolute', top: 14, left: 14, zIndex: 9999, padding: '7px 18px', borderRadius: 6, border: '1px solid #1e3a5c', background: '#132236', color: '#64b5f6', fontWeight: 700, fontSize: 12, cursor: 'pointer', fontFamily: "'Courier New', monospace", letterSpacing: '.05em' }}>
          ← Back to SCADA
        </button>
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', overflow: 'hidden', backgroundColor: bgColor, transition: 'background-color 0.6s ease', fontFamily: "'Courier New', monospace" }}>
      <TopStatusBar levels={levels} flows={flows} conicLevel={conicLevel} valveStates={valveStates} />
